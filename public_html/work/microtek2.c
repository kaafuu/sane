...
change_definition(Microtek2_Scanner *ms)
{
  ...
  buf_size = 1;
    ...
    if (usbflag)
      status = usb_read_ioctl(ms, (u_char *)&cmd, &buf, &buf_size);
    else
      status = sanei_scsi_cmd(ms->sfd, cmd, sizeof (cmd), &buf, &buf_size);
#if 0

  if (s->dev->interface == NEC_INTERFACE_SCSI)
    status = sanei_scsi_cmd (s->fd, cmd, sizeof (cmd), &buf, &buf_size);
  else if (s->dev->interface == NEC_INTERFACE_USB_M)
    status = usb_read_ioctl(s, (u_char *)&cmd, &buf, &buf_size);
  else
...
static SANE_Status
scsi_set_window1(Microtek2_Scanner *ms, int n) {   /* n windows, not yet */
                                                  /* implemented */
    SANE_Status status;
    u_int8_t *setwindow;
    int size;
...
#define POS  (setwindow + SW_BODY_P(n-1))
    ...
    SW_WNDID(POS, n-1);
    SW_XRESDPI(POS, 0x258);
    SW_YRESDPI(POS, 0xf0);
    SW_XPOSTL(POS, 0);
    SW_YPOSTL(POS, 0xfffffd1c);
    SW_WNDWIDTH(POS, ms->width_dots);
    SW_WNDHEIGHT(POS, 0x32);
    SW_THRESHOLD(POS, ms->threshold);
    SW_IMGCOMP(POS, ms->mode);
    SW_BITSPERPIXEL(POS, 0x0e);
    ...
    SW_LENS(POS, 0);                                  /* ???? */
    SW_INFINITE(POS, 0);
    SW_INFINITE_OFF(POS, 0);
    SW_STAY(POS, ms->stay);
    SW_RAWDAT(POS, 1);
    SW_QUALITY(POS, ms->quality);
    SW_QUALITY_OFF(POS, 0);
    SW_FASTSCAN(POS, ms->fastscan);
    SW_FASTSCAN_OFF(POS, 0);
...
#else
        dump_buf("setwindowcmd", setwindow, 10);
        dump_buf("setwindowheader", setwindow + 10 ,69);
	/*        dump_buf("setwindowbody", setwindow + 18 ,61);*/
#endif
      }
...
static SANE_Status
dump_buf (char *message, SANE_Byte *buf, size_t buf_size)
{
  char str[128];
  char *p;
  int flag, pos, delimiter;
...
usb_read_ioctl(int sfd, u_char cdb[], void *buf, size_t *length)
{
  ...
  creq.addr = 0;
  creq.request.bmRequestType = 0xc0;
  creq.request.bRequest = cdb[0];
  creq.request.wValue[0] = cdb[2];
  creq.request.wValue[1] = 0;
  creq.request.wIndex[0] = 0;
  creq.request.wIndex[1] = 0;
  creq.request.wLength[0] = *length & 0xff;
  creq.request.wLength[1] = (*length << 8) & 0xff;
  ...
  DBG(20, "\n[Setup Packet] %02x %02x %02x %02x %02x %02x %02x %02x\n"
      ...
      creq.request.bmRequestType,
      creq.request.bRequest,
      creq.request.wValue[0], creq.request.wValue[1],
      creq.request.wIndex[0], creq.request.wIndex[1],
      creq.request.wLength[0], creq.request.wLength[1],
      creq.addr, creq.data, creq.flags, creq.actlen);
...
usb_write_ioctl(int sfd, u_char cdb[], void *buf, size_t *length)
{
  ...
  creq.addr = 0;
  creq.request.bmRequestType = 0x40;
  creq.request.bRequest = cdb[0];
  creq.request.wValue[0] = cdb[2];
  creq.request.wValue[1] = 0;
  creq.request.wIndex[0] = 0;
    ...
    if ( md->model_flags & MD_READ_CONTROL_BIT )
      {
        ...
        DBG(30, "sane_start: ms->control_bytes=%p, malloc'd %d bytes\n",
      ...
      for ( pixel=0; pixel < mi->geo_width; ++pixel)
        {
          for ( color=0; color < 3; ++color )
            {
              offset = mi->color_sequence[color] * mi->geo_width
                       + pixel;
              img_val = *(md->shading_table_w + offset * 2 );
              img_val += *(md->shading_table_w + offset * 2 + 1)* 256.0;
              img_val /= 16.0;
              fputc((unsigned char)img_val, outfile_w);
              if ( md->shading_table_d != NULL )
                {
                  img_val = *(md->shading_table_d + offset * 2 );
                  img_val += *(md->shading_table_d + offset * 2 + 1)* 256.0;
                  img_val /= 16.0;
...
write_cshading_pnm(Microtek2_Scanner *ms)
{
  ...
  Microtek2_Device *md;
  Microtek2_Info *mi;
  int pixel, color, line, offset, img_val, img_height=30;
  ...
  switch( mi->data_format )
    {
      ...
      case MI_DATAFMT_LPLSEGREG:
        DBG(1, "prepare_shading_data: wordsize==1 incorrect supported\n");
        if ( ms->lut_entry_size == 1 )
          {
            for ( color = 0; color < 3; color++ )
              {
                for ( i = 0; i < mi->geo_width; i++ )
                  {
                    ...
                    value /= lines;
                    *((u_int8_t *) *data + color * mi->geo_width + i) =
...
calc_cx_shading_line(Microtek2_Scanner *ms)
{
    Microtek2_Device *md;
    SANE_Status status;
    u_int8_t *current_byte, *buf;
    u_int8_t color, factor;
    u_int8_t *shading_table_pointer;
    ...
    /*  calculating the mean pixel values over the shading lines  */
        ...
        for (i = 0; i < shading_line_pixels; i++)
          {
            ...
            *shading_table_pointer = (u_int8_t) (sum
                                    / (float) md->shading_length
                                    / (float) factor);
